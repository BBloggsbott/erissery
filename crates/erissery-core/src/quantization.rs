...
fn pad_to_block_size(data: &[f32], block_size: usize) -> Vec<f32> {
    ...
    padded.extend(std::iter::repeat(0.0f32).take(pad_size));
    padded
...
pub fn quantize_safetensors_q8_0(tensors: &SafeTensors) -> Result<Vec<QuantizedTensor>> {
    ...
    for (name, view) in tensors.tensors() {
        ...
        if f32_data.len() != num_elements {
            bail!("Tensor '{}': expected {} elements from shape {:?}, \
                 but decoded {} f32 values. Dtype was {}.",
                name, num_elements, shape, f32_data.len(), dtype)
        }
    ...
    println!("Done! Quantized {}/{} tensors.", results.len(), tensors.len());
    Ok(results)
